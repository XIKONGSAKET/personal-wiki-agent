...
### 第六步：安装 Claudian 插件（手动安装）也可以在第八步后再进行这一节，没差别
...
这个项目不仅仅是一键安装，更重要的是理解它的工作方式。以下是你需要知道的几个关键点：
...
**④ 提问（query）**
当你需要找某个知识时，可以使用 query 命令，但是我更建议你直接用自然语言：

> /query_wiki 你的问题
> 请你根据wiki中的信息回答我，XXXXXXXX（你的问题）
...
如果你想调整 AI 的行为方式（比如页面写作风格、分类规则），可以直接编辑 Vault 根目录下的 CLAUDE.md 文件。你的修改会立即生效，因为 AI 每次工作时都会读取它。
